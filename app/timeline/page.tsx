          ...
          {timeline.map((event, index) => (
            <article key={`${event.year}-${event.title}`} className="timeline-event">
              ...
              <div className="timeline-content">
                ...
                {'projectId' in event && event.projectId && <a href={`/directory?project=${event.projectId}`} className="text-link mt-5">View project record →</a>}
                {'policyId' in event && event.policyId && <a href={`/policy?record=${event.policyId}`} className="text-link mt-5">View policy record →</a>}
              </div>
            </article>
          ))}
        </div>
        <div className="mt-12 border border-line bg-white p-6 sm:flex sm:items-center sm:justify-between sm:gap-8 sm:p-8"><div><p className="eyebrow text-blue">This timeline is selective</p><p className="mt-2 max-w-2xl text-sm leading-6 text-slate">It prioritizes actions tied to the launch directory. Help add a well-sourced milestone or correction.</p></div><a href="/contact" className="button-secondary mt-5 shrink-0 sm:mt-0">Suggest a milestone</a></div>
      </section>
    </main>
  );
}
