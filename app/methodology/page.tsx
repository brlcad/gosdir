...
const criteria = [
  ['01', 'Government provenance', 'Official evidence establishes government creation, commissioning, funding, sponsorship, or direct stewardship of the work.'],
  ['02', 'Public source', 'The canonical source repository must be accessible without an account or special permission.'],
  ['03', 'Eligible license', 'The project uses an OSI-approved license or carries a clear public-domain dedication.'],
    ...
    <main>
      ...
      <section className="page-shell py-16 lg:py-20">
        <div className="grid gap-10 lg:grid-cols-[.75fr_1.25fr]">
          <div><p className="eyebrow text-blue">Inclusion standard</p><h2 className="section-title">Four checks before a record ships.</h2><p className="mt-5 text-sm leading-6 text-slate">Each verified record brings together government provenance, public source, an eligible license, and primary evidence.</p></div>
      ...
      <section className="border-y border-line bg-white">
        <div className="page-shell py-16 lg:py-20">
          <div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow text-blue">Moderated ingestion</p><h2 className="section-title">Automation finds. People decide.</h2><p className="mt-5 text-sm leading-6 text-slate">Discovery jobs open or update candidate issues. Editors publish qualified records through reviewed code changes, applying the same evidence standard to every suggestion.</p></div><ol className="grid gap-px border border-line bg-line sm:grid-cols-2">{pipeline.map(([number, title, body]) => <li key={number} className="bg-paper p-6"><span className="grid size-8 place-items-center rounded-full bg-blue font-mono text-xs font-black text-white">{number}</span><h3 className="mt-6 font-display text-xl font-black tracking-tight">{title}</h3><p className="mt-3 text-sm leading-6 text-slate">{body}</p></li>)}</ol></div>
      ...
      <section className="page-shell py-16 lg:py-20">
        ...
        <div className="mt-10 grid gap-5 border-l-4 border-signal bg-ink p-7 text-white sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="eyebrow text-signal">How to read coverage</p><p className="mt-3 max-w-3xl text-sm leading-6 text-white/65">Coverage grows as sources are reviewed. Each record links to primary evidence; follow those sources for current policy details.</p></div><a href="/guide" className="button-light">Use the release guide</a></div>
      </section>
    </main>
  );
}
